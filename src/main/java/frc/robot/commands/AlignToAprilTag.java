 ...
 * 
 * This command:
 * 1. Finds the closest AprilTag (or uses Limelight-detected tag)
 * 2. Calculates target position with optional offset (LEFT/RIGHT/CENTER)
 * 3. Uses PID control to drive the robot to the target pose
 * 4. Rotates to face opposite the tag (facing the tag)
 * 
...
public class AlignToAprilTag extends Command {
     ...
     */
    public AlignToAprilTag(CommandSwerveDrivetrain drivetrain, LimelightSubsystem limelight, AlignPosition alignPosition) {
        ...
        pidRotate = new PIDController(
            DrivetrainConstants.ALIGN_ROTATION_KP,
            0,
            DrivetrainConstants.ALIGN_ROTATION_KD
        );
    ...
    public void initialize() {
        ...
        if (limelightTagID != 0 && AprilTagMaps.aprilTagMap.containsKey(limelightTagID)) {
            ...
            System.out.println("[AlignToAprilTag] Limelight tag " + limelightTagID + " not in map, using odometry: " + targetTagID);
        }
        ...
        System.out.println("[AlignToAprilTag] Target Pose: X=" + targetPose.getX() + 
                          ", Y=" + targetPose.getY() + 
                          ", Yaw=" + Math.toDegrees(targetPose.getRotation().getRadians()) + "°");
        ...
        SmartDashboard.putNumber("AlignToAprilTag/TargetYaw", targetPose.getRotation().getDegrees());
    }
    ...
    public void execute() {
        ...
        SmartDashboard.putNumber("AlignToAprilTag/CurrentYaw", currentPose.getRotation().getDegrees());
