...
/**
 * This class controls the rotational heading of the drivetrain seperately
 * from translation control.
 * 
 * Some features I added:
 * 1. State Machine: SNAP (high gains to quickly reach target) vs MAINTAIN (low gains to hold)
 * 2. Automatic state transitions: After snapping to target, automatically switches to maintain
 * 3. Separate from drivetrain: Can be used with any translation source (driver input, auto path)
 * 
 * Usage:
 * - Call setGoal() to set target heading in degrees (field-relative)
 * - Call update() each loop with current heading to get rotation output
 * - The output is a normalized value (-1 to 1) representing rotational velocity demand
 */
public class SwerveHeadingController {
     ...
     * 
     * This should be called every robot loop with the current heading.
     * The output is a normalized rotation demand (-1 to 1) that should be
     * multiplied by max angular velocity before applying to the drivetrain.
     * 
     ...
     * Clears accumulated integral error and resets state
     */
    public void reset() {
     ...
     * Call this from a subsystem's periodic() method
     */
